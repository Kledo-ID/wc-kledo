<?php
/**
 * Plugin bootstrap: environment checks, constants, and loading the main plugin.
 *
 * @package wc-kledo
 */
...
class WC_Kledo_Loader {
	...
	private function get_wc_required_message(): string {
		/* translators: %s: WooCommerce product name HTML link. */
		return sprintf( esc_html__( 'WooCommerce Kledo requires %s to be installed and active.', 'wc-kledo' ), '<a href="https://woocommerce.com/" target="_blank">WooCommerce</a>' );
	}
		...
		if ( ! $this->is_wc_compatible() ) {
			$this->add_admin_notice(
				...
				sprintf(
					...
					'<a href="' . esc_url( admin_url( 'update-core.php' ) ) . '">',
					'</a>',
					'<a href="' . esc_url( 'https://downloads.wordpress.org/plugin/woocommerce.' . WC_KLEDO_MIN_WC_VERSION . '.zip' ) . '">',
					'</a>'
				)
			);
		}
	...
	protected function deactivate_plugin(): void {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Removes activation screen query arg after deactivation; not user-submitted data.
